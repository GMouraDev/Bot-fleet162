...
export_url = os.getenv('FROTA162_MULTAS_EXPORT_URL')
...
print("Iniciando processo de exportação de multas...")
...
try:
    ...
    print("Exportando dados de multas...")
    ...
    with tqdm(total=100, desc="Processando dados") as pbar:
        ...
        try:
            existing_data = pd.read_excel(f"{resources_dir}/Multas162.xlsx")
        ...
        # Salva arquivo final
        output_path = f"{resources_dir}/Multas162.xlsx"
        unique_data.to_excel(output_path, index=False)
        print(f"Planilha de multas salva em: {output_path}")
        
        pbar.update(100)
    
    print(f"✅ Processo concluído! Novas multas importadas: {len(new_entries)}")
    print(f"📊 Total de registros únicos: {len(unique_data)}")
